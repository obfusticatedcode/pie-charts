 ...
 * @memberof Graph/helper
 */
function _initializeLinks(graphLinks,highlightDirection=false) {
    return graphLinks.reduce((links, l) => {
        ...
        if(highlightDirection==='INWARD'){
            links[source][target] = Number(l[l._value])||1//= links[target][source] = l.value || 1;
        }else if(highlightDirection==='OUTWARD'){
            links[target][source] = Number(l[l._value])||1//= links[target][source] = l.value || 1;
        }else{
            links[target][source]  = links[source][target] = Number(l[l._value])||1;
        }
...
function buildLinkProps(link, nodes, links, config, linkCallbacks, highlightedNode, highlightedLink, transform) {
    ...
    const curveIt=(config.link.directed&&highlightedNode)?(highlightedNode===source||highlightedNode===target)&&(links[target]&&links[target][source]):false
    const d = buildLinkPathDefinition({ source: { x: x1, y: y1 }, target: { x: x2, y: y2 } }, curveIt?'CURVE_SMOOTH':(link.type||config.link.type));

    if (highlight&&(highlightedNode===source)) {
        let customColor=typeof(config.link.highlightColor)==='function'?config.link.highlightColor(link):config.link.highlightColor
        stroke = config.link.highlightColor === CONST.KEYWORDS.SAME ? config.link.color : customColor;
    };
...
function initializeGraphState({ data, id, config }, state) {
    ...
    graph.links = data.links.map(l => {
        let add_props={}
        try{
            add_props._value=config.link.value
        }finally{
            return Object.assign(add_props, l)
        }
    });

    let newConfig = Object.assign({}, utils.merge(DEFAULT_CONFIG, config || {}));
    let nodes = _initializeNodes(graph.nodes);
    let links = _initializeLinks(graph.links,config.link.highlightDirection); // matrix of graph connections
